...
' --------------------------------------------------------------------------------
		...
		' http://d.hatena.ne.jp/replication/20091117/1258418243 より丸写し
		' ------------------------------------------------------------
		...
		' Windows7 以降の判定
		Function IsPostWin8
			IsPostWin8 = left(GetWinVersion, 3) >= 6.2
		End Function
		' Windows7 以降の判定
		Function IsPostWin7
			IsPostWin7 = left(GetWinVersion, 3) >= 6.1
		End Function
		' Vista 以降の判定
		Function IsPostWinVista
			IsPostWinVista = left(GetWinVersion, 3) >= 6.0
		End Function
		' OS バージョン
		Function GetWinVersion
			...
			With GetObject("winmgmts:" & "{impersonationLevel=impersonate}!\\.\root\cimv2")
				...
				For Each os in .ExecQuery("SELECT * FROM Win32_OperatingSystem")
					GetWinVersion = os.Version
					Exit Function
				Next
			End With
		End Function
		' OS アーキテクチャ
		Function GetWinArchitecture
			GetWinArchitecture = "32-bit"
			With GetObject("winmgmts:" & "{impersonationLevel=impersonate}!\\.\root\cimv2")
				Dim os
				For Each os in .ExecQuery("SELECT * FROM Win32_OperatingSystem")
					If os.Version >= 6.0 Then
						GetWinArchitecture = os.OSArchitecture
						Exit Function
					End If
				Next
			End With
